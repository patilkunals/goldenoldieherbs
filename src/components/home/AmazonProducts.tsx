    ...
    <section id="products" className="py-16 bg-sand">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setB2bOpen(true)} className="px-4 py-2 rounded-md bg-gold text-charcoal font-semibold">B2B Wholesale Inquiry</button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map(p => (
            <motion.article key={p.id} whileHover={{ y: -6 }} className="rounded-xl p-4 bg-white/60 border shadow-sm">
              <SafeImage src={p.image} alt={p.name} className="w-full h-44 object-cover rounded-md mb-3" />
              ...
              <div className="flex items-center gap-3">
                <a href={p.amazonLink} target="_blank" rel="noreferrer" className="inline-flex items-center px-3 py-2 rounded-md bg-primary text-sand font-medium">Buy on Amazon India</a>
                {p.b2bAvailable && <span className="text-sm px-2 py-1 rounded-md bg-gold text-charcoal font-semibold">B2B</span>}
              </div>
            </motion.article>
          ))}
        </div>
